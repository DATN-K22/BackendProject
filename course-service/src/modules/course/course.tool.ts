...
export class CourseTool{
    private readonly QUERY_STOP_WORDS = new Set([
        "i", "im", "i'm", "me", "my", "we", "our", "you", "your",
        "a", "an", "the", "and", "or", "to", "for", "of", "in", "on", "with", "about",
        "please", "find", "give", "show", "need", "want", "learn", "course", "courses",
        "that", "this", "it", "is", "are", "be", "as", "by", "from", "into", "currently",
    ]);

    private readonly QUERY_CONSTRAINT_WORDS = new Set([
        "cheap", "fast", "quick", "beginner", "beginnerfriendly", "friendly", "highly", "rated",
        "weekend", "weekendonly", "practical", "handson", "hands", "exactly", "specific", "best",
    ]);
    ...
    private heuristicScore(course: any, query: string): number {
        ...
        const queryTokens = this.tokenizeQuery(query)
            .filter((token) => !this.QUERY_STOP_WORDS.has(token) && !this.QUERY_CONSTRAINT_WORDS.has(token));
    ...
    private buildRelaxedQueries(query: string) {
        ...
        const quotedPhrases = this.extractQuotedPhrases(query);
        const topicTokens = baseTokens.filter((token) =>
            !this.QUERY_STOP_WORDS.has(token) && !this.QUERY_CONSTRAINT_WORDS.has(token)
        );
    ...
    @Tool({
        ...
        description: "Search courses by keyword against title and short_description. Supports optional filters and deterministic sort for better course selection.",
        parameters: z.object({
            ...
            courseLevel: z.enum(["Beginner", "Intermediate", "Advanced", "Expert", "AllLevels"]).optional().describe("Optional exact course level filter"),
            sortBy: z.enum(["relevance", "rating", "price", "heuristic"]).default("heuristic").describe("Sort strategy for the final result list"),
            includeSuggestions: z.boolean().default(true).describe("When true, include suggested follow-up queries if result is empty"),
        })
    })
        ...
        return {
            content: [{
                ...
                text: stringify({
                    ...
                    suggested_queries: isEmpty && (includeSuggestions ?? true)
                        ? this.buildSearchSuggestions(query)
                        : [],
                    items: result,
                }),
            }],
    ...
    async getCourseSyllabus(
        ...
        const result = chapters.map((chapter, chapterIndex) => {
            ...
            const lessons = typeof maxLessonsPerChapter === "number"
    ...
    async findSyllabusLecture(
        ...
        const selected = mode === "all"
            ? matches
            : mode === "last"
                ? (matches.length ? [matches[matches.length - 1]] : [])
