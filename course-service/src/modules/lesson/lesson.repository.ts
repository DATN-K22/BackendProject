  ...
  async getChapterItemByIdWithValidateUserEnrollment(itemId: string, userId: string) {
    ...
    if (chapterItem.item_type === 'lesson' && chapterItem.lesson) {
      return {
        ...
        is_free: chapterItem.lesson.is_free,
        resources: (chapterItem.lesson.resources ?? []).map((resourceId) => resourceId.toString()),
        chapter,
        isFinished
      }
    }
    ...
    if (chapterItem.item_type === 'lab' && chapterItem.lab) {
      return {
        ...
        instruction: chapterItem.lab.instruction ?? undefined,
        resources: (chapterItem.lab.resources ?? []).map((resourceId) => resourceId.toString()),
        chapter,
        isFinished
      }
    }
